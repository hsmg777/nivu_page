...
/* Reveal simple (igual que tenías) */
function useReveal() {
  useEffect(() => {
...
export default function Hero() {
  ...
  // Parallax: habilitar solo si NO reduce motion, memoria decente y desktop
  const enableParallax = useMemo(() => {
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    const mem = navigator.deviceMemory || 4;      
    const desktop = window.matchMedia("(min-width:1024px)").matches;
    return !reduce && desktop && mem >= 4;
  }, []);

  // Activar/desactivar listener solo cuando el hero está en viewport
  const [inView, setInView] = useState(true);
  useEffect(() => {
    const io = new IntersectionObserver(
      ([e]) => setInView(e.isIntersecting),
      { rootMargin: "0px 0px -50% 0px" } // cuando ya pasó la mitad, lo apagamos
    );
  ...
  useEffect(() => {
    if (!enableParallax || !inView || !imgRef.current) return;
  ...
  }, [enableParallax, inView]);

  return (
    <section
      id="inicio"
      ref={sectionRef}
      className="relative min-h-[100svh] w-full overflow-hidden bg-[#7ec6f0]"
    >
      {/* Fondo responsive: AVIF; ocupa el flujo para LCP real */}
      <picture className="absolute inset-0 z-0 block">
        {/* Desktop ≥768px */}
        <source media="(min-width:768px)" type="image/avif" srcSet="/images/hero_bg.avif" />
        {/* Mobile */}
        <source type="image/avif" srcSet="/images/bg_hero_cell.avif" />

        {/* Fallback temporal (cuando tengas JPG cámbialo aquí) */}
        <img
          ref={imgRef}                       // <- para tu parallax
          src="/images/bg_hero_cell.avif"    // (luego cámbialo a .jpg si exportas fallback)
          sizes="100vw"
          alt=""
          fetchPriority="high"
          decoding="async"
          className="h-full w-full object-cover md:object-[center_top_-210px]"
      ...
      <div className="flex h-[100svh] w-full flex-col items-center justify-end px-6 pb-24 text-center sm:pb-28">
        ...
        <a
          ...
          className={[
            ...
            // Evitar blur caro en móvil; sólo desktop
