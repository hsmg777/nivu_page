...
import { LazyMotion, m, useScroll, useTransform, useReducedMotion } from "framer-motion";
import CubeButton from "./CubeButton";

const loadFeatures = () => import("framer-motion").then(m => m.domAnimation);

export default function Caption({
  bgSrc = "/images/bg_caption.avif",   // <- AVIF en public/images
  ...
  // Solo transform + opacity (GPU). Sin blur animado.
  ...
  return (
    <section
      ref={wrapperRef}
      className="relative"
      style={{ minHeight: `${(1 + extraScreens) * 100}vh` }}
    >
      <div className="sticky top-0 h-screen overflow-hidden">
        <LazyMotion features={loadFeatures}>
          {/* Imagen de fondo (solo AVIF) */}
          <m.img
            src={bgSrc}
            alt=""
            loading="lazy"
